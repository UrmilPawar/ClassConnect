...
const redisHost = '4.188.72.105'; // Use your Azure Redis host IP or hostname
...
redisSubscriber.subscribe(channel, (message) => {
    ...
    if(message.action=='connect')
    {
        ...
        console.log(`Added ${t_id} in 8080 set`);
    ...
    else if (message.action=='start')
    {
        ...
        if (students.has(t_id)) {                                                         // If there are students connected to this t_id
            ...
            console.log(`No students connected for t_id: ${t_id} on server ${PORT}`);
        }
    }
});
...
wss.on('connection', (ws,req) => {
    ...
    const message = JSON.stringify({action: 'check',url: 'ws://localhost:8080',count: connections});
